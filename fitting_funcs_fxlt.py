...
@njit(fastmath=True)
def IRBF_njit(f, params, width, n, lt):
    """
    ...
    lt:     scalar loss tangent from material properties
    returns (Nf,)
    """
    C, K_Ray, K_Mie, a_RM = params
...
@njit(fastmath=True)
def IRBF_alpha_njit(f, params, n, lt):
    C, K_Ray, K_Mie, a_RM = params
...
def IRBF_batch_params(f, samples, width, n, lt):
    Ns = samples.shape[0]
    Nf = f.size
    out = np.empty((Ns, Nf), dtype=np.float64)
    for i in prange(Ns):
        out[i, :] = IRBF_njit(f, samples[i, :], width, n, lt)
    return out


@njit(parallel=True, fastmath=True)
def alpha_batch_params(f, samples, n, lt):
    Ns = samples.shape[0]
    Nf = f.size
    out = np.empty((Ns, Nf), dtype=np.float64)
    for i in prange(Ns):
        out[i, :] = IRBF_alpha_njit(f, samples[i, :], n, lt)
    return out


@njit(fastmath=True)
def IRBF_components_njit(f, params, width, n, lt):
    C, K_Ray, K_Mie, a_RM = params
...
@njit(parallel=True, fastmath=True)
def components_batch_params(f, samples, width, n, lt):
    Ns = samples.shape[0]
    Nf = f.size
    ...
    for i in prange(Ns):
        Abs_out[i, :], Ray_out[i, :], Mie_out[i, :], res_out[i, :] = \
            IRBF_components_njit(f, samples[i, :], width, n, lt)
...
def loglike(params, f, t, width, err, n_eff, lt):
    model = IRBF_njit(f, params, width, n_eff, lt)
    loglike = -0.5 * np.sum((t - model)**2 / err**2 + np.log(2 * np.pi * err**2))
    return loglike

def fitting(f, t, width, err, n_eff, lt, nlive, dlogz, ncpu=1, prior1=None, prior2=None, priortype=None):
    if prior1 is None:
        prior1     = [-5,  100,  100, 0]
    if prior2 is None:
        prior2     = [2, 4000, 4000, 1]
    if priortype is None:
        priortype = ["LU", "U", "U", "U"]
    loglike_args = [f, t, width, err, n_eff, lt]

    pool = Pool(ncpu) if ncpu > 1 else None
    try:
        print('Running Nested Sampling...')
        sampler = dynesty.NestedSampler(loglike, ptform, ndim=4, nlive=nlive,
